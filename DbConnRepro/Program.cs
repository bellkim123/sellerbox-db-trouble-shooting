...
public static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            Console.WriteLine("❌ TcpProbeStorm requires --host (and optionally --port).");
            return 1;
        }
    ...
    // 1) TCP 포트 프로브 폭탄 (인증 없이 붙었다 바로 끊음)
    private static async Task RunTcpProbeStormAsync(Arguments args, Metrics metrics, CancellationToken ct)
    {
        Console.WriteLine("Running TcpProbeStorm… (will generate unauthenticated closes)");
            ...
            workers.Add(Task.Run(async () =>
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        int ms = Math.Max(1000, args.CancelMs > 0 ? args.CancelMs : 1500);
                        innerCts.CancelAfter(ms);

                        await tcpClient.ConnectAsync(args.Host!, args.Port, innerCts.Token);
                        metrics.IncrementSuccess();
                    }
                    catch (OperationCanceledException oce)
                    {
                        metrics.IncrementCancelled(oce); // 의도된 취소
    ...
    // 2) 핸드셰이크 직전 취소 폭탄 (짧은 ConnectTimeout + OpenAsync 취소)
        ...
        for (int i = 0; i < args.Concurrency; i++)
        {
            workers.Add(Task.Run(async () =>
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    if (args.CancelMs > 0) cancelOpen.CancelAfter(args.CancelMs); // 100~300ms 같이 매우 짧게

                    try
                    {
                        await connection.OpenAsync(cancelOpen.Token); // 핸드셰이크 중 취소 → unauthenticated로 보일 수 있음
                        metrics.IncrementSuccess();
                    }
                    catch (OperationCanceledException oce)
                    {
    ...
    // 3) 짧은 ConnectTimeout + 무백오프 재시도(안티패턴)
            ...
            workers.Add(Task.Run(async () =>
            {
                while (!ct.IsCancellationRequested)
                {
                    for (int attempt = 0; attempt < 3 && !ct.IsCancellationRequested; attempt++)
                    {
                        ...
                        try
                        {
                            await connection.OpenAsync(ct); // Connect Timeout이 짧으면 실패/취소 급증
                            metrics.IncrementSuccess();
                            break;
                        }
                        catch (OperationCanceledException oce)
    ...
    // 5) 헬스체크 남발 (SELECT 1을 높은 주기와 병렬로)
    ...
    // 6) 잘못된 비밀번호로 인증 실패 반복 (주의: 계정 잠김 위험)
        ...
        for (int i = 0; i < args.Concurrency; i++)
        {
            workers.Add(Task.Run(async () =>
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        await connection.OpenAsync(ct); // 인증 실패 반복
                        metrics.IncrementSuccess();
                    }
                    catch (OperationCanceledException oce)
                    {
...
Usage:
  dotnet run -- [--scenario <name>] [--conn <connectionString>|ENV DB_CONN] [--host] [--port]
                [--concurrency N] [--durationSec S] [--intervalMs MS] [--cancelMs MS] [--retryDelayMs MS] [--badPwd true]

Scenarios:
  TcpProbeStorm            : TCP로 3306 포트만 두드리고 즉시 끊음(unauthenticated 대량 유발)
  ...
  WrongCredentialsBurst    : 잘못된 자격증명으로 인증 실패 반복 (계정 잠김 위험)
